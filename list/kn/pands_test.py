...
# 计算协方差
'''
Series.cov（min_periods = None）：计算列的协方差, 不包括NA / null值. min_periods表示每个列对所需的最小观测值数
'''
...
# 使用rank方法排序,根据每行数据的大小返回名次
'''
DataFrame.rank（axis = 0，method ='average'，numeric_only = None，na_option ='keep'，ascending = True，pct = False ）：
返回数据的排名. axis选择行或列，method ='average'表示对平均值排名（可以取一下几个值'average'，'min'，'max'，'first'，'dense'），
